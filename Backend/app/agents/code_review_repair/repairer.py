...
def build_code_review_repair_prompt(
    *,
    ...
    package_install_instruction = (
        "For pnpm_install, edit package.json first and then call the pnpm_install_frontend "
        "tool exactly once to regenerate pnpm-lock.yaml; never edit pnpm-lock.yaml with file "
        "tools. Calling pnpm_install_frontend is mandatory — the lockfile will not be "
    ...
    lockfile_clause = (
        "The denied_paths entry frontend/pnpm-lock.yaml:file_write only forbids editing the "
        "lockfile with file tools; the pnpm_install_frontend tool is the authorized way to "
        "regenerate it and must be called when repair_actions include pnpm_install. "
        if PNPM_INSTALL_REPAIR_ACTION in authorized_actions
        else "The denied_paths entry frontend/pnpm-lock.yaml:file_write forbids editing the "
        "lockfile with file tools; no lockfile regeneration is authorized in this packet. "
    )
    return (
