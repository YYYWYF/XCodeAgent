...
def _agent_runtime_generation_prompt(
    *,
    ...
    return (
        ...
        "create_deep_agent. Preserve each declared Tool's business schema and Java Endpoint "
        "identity, but treat Java gateway transport as deferred unless the task or current template "
        "supplies an explicit supported boundary. Never invent Java source, a client, URL, Header, "
        "Token name, credential contract, or response envelope. When transport is absent, generate a "
        "fail-closed Tool boundary and report the deferred live integration without blocking the Agent "
        "module or focused test. The browser must never call the Python sidecar directly. All writes "
        "must stay inside each task's allowed_paths under "
