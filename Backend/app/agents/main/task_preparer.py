...
def _combined_task_preparation_prompt(
    ...
    return (
        ...
        "- backend endpoint source_refs.entity_ids: the exact non-empty subset of confirmed "
        "entities implemented by this task; do not copy unrelated entities from the Unit\n"
...
def _common_task_preparation_rules() -> str:
    ...
    return (
        ...
        "and verification_commands=[]. Every backend:endpoint:* task must also include "
        "source_refs.entity_ids as the exact non-empty subset of confirmed entities that task "
        "implements; never copy unrelated Unit entities. Return one JSON object with workspace_analysis and "
