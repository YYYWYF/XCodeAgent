...
def _task_preparation_prompt(
    ...
    return (
        ...
        "when no separate pre-check task is created), and cover the 11 generated files in its "
        "acceptance_criteria. The phases are the execution order inside the module task, not "
        ...
        "（*.test.*、*.spec.*、Java 测试类等）。前后端任务的 acceptance_criteria 只描述"
        "代码产物本身（目标文件已生成或修改、字段与契约一致、页面按 page_goal 实现），"
        "verification_commands 必须留空；所有验证统一由后续集成测试阶段负责。\n"
        ...
        "acceptance_criteria. Frontend page tasks must additionally follow the "
