...
def _task_preparation_prompt(
    ...
    return (
        ...
        "When executable_details.database_planning_context.status is completed, treat its "
        "DatabasePlanningContext summaries and tables as the only real database structure. "
        "Use it together with endpoint_detail_plans and api_contracts to split database tasks "
        "from backend code tasks. Do not infer database tables or columns from "
        "ProjectPlan.data_sources. If database_planning_context is skipped or failed, keep "
        "the uncertainty explicit in database task risks and verification criteria.\n"
