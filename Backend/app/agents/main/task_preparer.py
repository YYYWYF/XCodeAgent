...
def _task_preparation_prompt(
    ...
    return (
        ...
        "and generates no business code. For each backend module (one endpoint/table), "
        "consolidate the four phases into ONE module generation task (owner: backend) that "
        "generates all 11 module files in the 4-phase order (object classes "
        "Entity/PO/DTO/Converter/Assembler → Repository/Mapper → ApplicationService → "
        "Controller); when the pre-check task exists, it must be listed in this task's "
        "dependencies. Do NOT split the four phases into separate tasks. Put all 11 generated "
        "files in the module task's change_scope (also add missing infrastructure files there "
        "when no separate pre-check task is created). A deterministic compiler derives engineering "
        "acceptance checks from that exact change_scope. The phases are the execution order inside the module task, not "
        "separate tasks. Follow its naming conversions "
        ...
        "业务验收标准复制到任务；acceptance_criteria 必须返回空数组，后端会根据 change_scope、"
        "allowed_paths、菜单元数据和正式 API 契约确定性生成纯工程验收点。"
        ...
        "The Spring Boot backend starts on port 8080; its base address is "
        ...
        "equivalent list items. Keep acceptance_criteria empty and never add generic build, test, "
        "runtime UI, permission, role, or other business acceptance statements.\n"
