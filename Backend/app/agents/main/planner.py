...
from app.agents.main.technical_plan_response import (
    invoke_technical_plan_model,
    parse_technical_plan_json,
)
...
def _technical_planning_prompt(
    ...
    return (
        ...
        "never emit an empty entity_ids array or invent a transport-only entity. A business Contract name is a "
        "concise user-facing Chinese name such as 订单管理; it is distinct from the stable id. "
...
def _invoke_live_chat_model(
    ...
    *,
    ...
    return _invoke_prompt_with_chat_model(
        ...
        on_token=on_token,
        technical_plan=isinstance(requirement_spec.get("confirmed_product_plan"), dict),
    )
...
def _invoke_prompt_with_chat_model(
    ...
    *,
    ...
    if technical_plan:
        return invoke_technical_plan_model(prompt, settings=settings, on_token=on_token)
...
def repair_technical_plan_api_contracts_with_chat_model(
    ...
    *,
    ...
    response = parse_technical_plan_json(response_text)
...
def plan_project_with_chat_model(
    ...
    *,
    ...
    if isinstance(requirement_spec.get("confirmed_product_plan"), dict):
        ...
        return create_technical_plan(
            requirement_spec,
            agent_plan=parse_technical_plan_json(agent_note),
            datasource_type=datasource_type,
        )
