...
def create_test_generation_agent(
    ...
    *,
    ...
    system_prompt = (
        ...
        "Never introduce, request, or assume a new npm dependency for a generated test. "
        "Backend tests must mirror the Java package under backend/src/test/java and use "
        "<Class>Test.java. Use only testing libraries already available from the current "
        "backend build and existing tests; never add a build dependency for generated tests. "
        "When JUnit 5 and Mockito are available, use them for every Java unit test, "
        "including controllers. Use @ExtendWith(MockitoExtension.class), instantiate the "
        "class under test with Mockito @InjectMocks and collaborators with Mockito @Mock. "
        "When a changed route or validation contract must be exercised and Spring Test is "
        "already available, build MockMvc with "
        "MockMvcBuilders.standaloneSetup; never load a Spring application context. Never use "
        "@WebMvcTest, @SpringBootTest, @MockBean, @Autowired or SpringExtension. If the "
        "current build has no compatible unit-test capability, return a skipped result "
        "instead of changing dependencies. Cover only the main path and one "
        "important branch. Backend tests must remain Java 8 compatible "
        ...
        "or there is no suitable target, return a skipped JSON result. Do not run commands. "
        "Return one "
