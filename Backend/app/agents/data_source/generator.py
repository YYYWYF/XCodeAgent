...
def _data_source_generation_prompt(
    *,
    ...
    return (
        ...
        "API design boundary: implementation_contract.api_design is the confirmed self-contained "
        "field mapping document for exactly one internal Endpoint. The internal api_contract remains authoritative "
        "for Controller method, path, request, and response; api_design may only specify source "
        "fields, field mappings, and multiline business descriptions. Iterate every sourceFields entry. "
        "processingType=direct means a one-source assignment; single_field_description and "
        "multi_field_description require implementing the complete user-confirmed businessDescription "
        "using every declared source. Preserve request filter/write and response read direction. "
        "Never invent missing algorithms, joins, source fields, or upstream operations. If critical "
        "semantics are missing or conflicting, return failed with contract_mismatch and a concrete "
        "change_request for user clarification before implementation. Pure business_description has "
        "no physical sources. Source snapshots are confirmation-time non-sensitive facts. For external API "
        "sources, the selected operation controls only the upstream call. Prefer Spring Cloud OpenFeign for "
        ...
        "requestStructure and responseStructure as field/type structure, never as literal values. "
        "Use api_design.fieldMappings as the only internal-to-source field authority; if a required "
        "mapping cannot be implemented, return failed with failure_category=contract_mismatch rather "
        "than hard-coding or inventing a value. Persist the confirmed baseUrl directly as "
        ...
        "the base URL in Java constants. A business_description is only a concise field-level note "
        "for Request or Response behavior; preserve it as guidance without persisting control parameters "
        "or inventing field bindings. Deserialize the declared response root into typed transport "
        "DTOs and apply the confirmed fieldMappings records. Do not infer pagination or transformation rules "
        "from familiar names; implement only the confirmed direct mappings and written descriptions. Preserve declared success "
        "status codes and route upstream HTTP errors, "
        "timeouts, declared error paths, and deserialization failures through existing exception "
        "conventions.\n\n"
        "When implementation_contract.api_design contains a non-empty "
        "implementationDescription, use it as optional Endpoint-level guidance for how the "
        "interface should be implemented across the appropriate Controller, ApplicationService, "
        "Repository, or upstream layer. It is descriptive guidance only: it cannot change the "
        "confirmed TechnicalPlan method, path, parameters, schemas, mappings, or source facts. "
        "If it conflicts with those facts, preserve the confirmed contract and return a "
        "contract_mismatch failure rather than inventing a dependency or behavior.\n\n"
