...
def _data_source_generation_prompt(
    *,
    ...
    return (
        ...
        "the table schema or SQL DDL first. After generating the new module files, continue with the "
        "regular backend verification.\n"
        "--- END SKILL INVOCATION ---\n"
        "\n"
        "Never create temporary scripts for verification.\n"
        ...
        "or any other temporary script files to run build or test commands. Instead, use the "
        "`execute` tool directly to execute commands like `pnpm run build`, `pnpm run dev`, "
