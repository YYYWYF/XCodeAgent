    ...
    return {
        ...
        "workflowActions": {
            ...
            "values": {
                ...
                "start_api_design": (
                    "按 selectedApiContractId 和 selectedEndpointId 启动独立 API 设计，确认后继续当前 Endpoint 的开发链路。"
                ),
        ...
        "input": {
            ...
            "forwardedProps.workflowDebug": "Optional frontend debug resume settings with resumeFrom, requirementSpecPath, projectPlanPath, workspaceSnapshotPath, and buildTaskPlanPath.",
        },
        "output": {
...
async def build_workflow_response(
    *,
    ...
    request = request_inputs["request"]
    resume_from = request_inputs.get("resume_from") or resume_from
    ...
    *,
    ...
    for message in messages:
        ...
        yield encoder.encode(
            ToolCallResultEvent(
                messageId=str(
                    getattr(message, "id", "") or f"tool-result:{tool_call_id}"
                ),
                toolCallId=tool_call_id,
                content=result,
                role="tool",
...
def _is_command_tool(name: str) -> bool:
    ...
    return any(
        token in lowered
        for token in ("execute", "exec", "shell", "terminal", "command")
    )
...
def _workflow_start_node(
    ...
    if resume_from == "api_design":
        return "api_design"
    if resume_from == "api_design_readiness_gate":
...
def _workflow_node_detail(node_name: str, update: dict[str, Any]) -> dict[str, Any]:
    ...
    if node_name == "inspect_workspace":
        snapshot = update.get("workspace_snapshot_summary")
        manifest = (
            snapshot.get("file_manifest", {}) if isinstance(snapshot, dict) else {}
        )
        return {
            "message": (
                f"revision={update.get('workspace_revision')}，"
                f"files={manifest.get('total_files_indexed', 0)}，"
                f"snapshot={update.get('workspace_snapshot_path')}"
            ),
            "data": {
                "workspaceRevision": update.get("workspace_revision"),
