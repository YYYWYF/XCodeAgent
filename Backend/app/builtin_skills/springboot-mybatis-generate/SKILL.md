...
每个后端任务的 `description` 必须使用 `1. ...`、`2. ...` 形式的中文有序执行清单，并写出本阶段的精确目标路径和业务职责。文件存在性与 `add`/`modify` 的判定遵循上层公共 Task Rules；清单直接要求读取现有文件并判断是否满足完整业务需求，或依据已确认契约创建缺失文件。已经完全满足的现有文件保持不变，只有部分满足时才最小补齐缺失能力。执行阶段检查实时文件只用于防御 WorkspaceSnapshot 之后的变化；整项任务无需写入时应返回 `already_satisfied` 及有效证据。
