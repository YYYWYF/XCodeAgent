...
| `src/constants/resources.ts` | `/frontend/src/constants/resources.ts`（仅 auth，deterministic auth-guard Task） |
| `src/constants/routes.tsx` | `/frontend/src/constants/routes.tsx`（仅 auth，Build 平台预投影） |

**生成代码前，读取 `/.xcodeagent/template-generation-manifest.json` 的 `templateVariant` 和当前任务允许路径。** `main` 模板已由平台创建页面占位和 `BIZ_MENUS`；`auth` 模板由平台预投影路由注册，资源注册则由 deterministic auth-guard Task 唯一写入。不要把文件写到工作区根下的裸 `src/` 或 `Frontend/src/`，那会写到错误位置。
...
- `auth`：页面任务只生成 `src/pages/<PageKey>/index.tsx`；平台根据已确认的 `authorization_manifest` 预投影 `src/constants/routes.tsx` 的固定托管区，deterministic auth-guard Task 唯一写入 `src/constants/resources.ts`，并由平台创建页面占位文件。
