...
def write_planning_run_atomic(
    ...
    """供 Controller 原子写入轻量投影，并与 Pending 生命周期提交串行化。"""

    # 延迟导入避免 task_documents 的 DraftIdentity 依赖在模块加载期形成循环。
    # PlanningRun 终态与 Pending 必须共享提交顺序：先提交 cancelled 就禁止随后
    # 写 Pending；先提交 Pending 则由 Cancel 读取权威草稿并让确认态胜出。
