...
def _requirement_issues(
    ...
    for kind, target_id, capabilities, task_id in records:
        ...
        for capability in matched_capabilities:
            ...
            target_field = DELIVERABLE_TARGET_IDENTITY_FIELD_BY_KIND.get(
                expected_kind or ""
            )
            if target_field is None:  # pragma: no cover - Context 前置校验必须先拦截。
                raise RuntimeError(
                    f"validated generation requirement kind unexpectedly missing: "
                    f"{expected_kind!r}"
                )
