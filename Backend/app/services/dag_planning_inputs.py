...
class SequentialPlanningInputs(FrozenPlanningModel):
    ...
    def input_fingerprint(self) -> str:
        """对完整冻结 Planning Inputs 计算唯一摘要，供 Run 与 Confirm 共用。"""

        return _input_digest(self.model_dump(mode="json"))
    ...
    def requirements(self) -> UnitGenerationRequirements:
        ...
        return resolve_generation_requirements(
            ...
            endpoint_designs=plain_json(tuple(
                contract.content
                for contract in self.formal_contract_inputs.endpoint_api_designs
            )),
        )
