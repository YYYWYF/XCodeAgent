...
def endpoint_api_design_index(
    store: FrozenContractStore,
) -> dict[tuple[str, str], FrozenContract]:
    """按 apiContractId 与 endpointId 建立 Endpoint API Design 双键索引。"""

    result: dict[tuple[str, str], FrozenContract] = {}
    for contract in store.contracts.values():
        if contract.kind != "endpoint_api_design":
            continue
        key = (
            exact_manifest_id(
                contract.content.get("apiContractId"),
                "endpoint_api_design.apiContractId",
            ),
            exact_manifest_id(
                contract.content.get("endpointId"),
                "endpoint_api_design.endpointId",
            ),
        )
        if key in result:
            raise ContractCatalogBindingError(
                f"FrozenContractStore 中 Endpoint API Design {key[0]}/{key[1]} 不唯一。"
            )
        result[key] = contract
    return result
...
def endpoint_api_design_source_types(
    ...
    for mapping in manifest_sequence(
        ...
        if not isinstance(mapping, Mapping):
            raise ContractCatalogBindingError(
                "Endpoint API Design.fieldMappings 项必须为对象。"
            )
        source_fields = mapping.get("sourceFields")
        if source_fields is None:
            continue
        for source in manifest_sequence(
            source_fields,
            "Endpoint API Design.fieldMappings.sourceFields",
        ):
            if not isinstance(source, Mapping):
                raise ContractCatalogBindingError(
                    "Endpoint API Design.fieldMappings.sourceFields 项必须为对象。"
                )
...
def api_selectors(contract: FrozenContract, endpoint_indexes: set[int]) -> set[str]:
    """只允许读取 API 身份和精确 Endpoint，字段来源由 Endpoint API Design 提供。"""
