...
def compile_expected_unit_formal_source_refs(
    *,
    ...
    entities = contracts_by_identity(store, kind="entity_binding", identity_field="entity_id")
    ...
    for raw_requirement in generation_requirements:
        ...
        if requirement_kind == "backend.bootstrap":
            ...
            relevant_keys = {
                ...
                if any(
                    ...
                    and entities[entity_id].content.get("data_source_type") == source_type
                    for entity_id in manifest_sequence(
                        endpoints[key][0].content.get("entity_ids"),
                        f"API Contract {key[0]}.entity_ids",
                    )
                )
            }
        ...
        for ref_id, endpoint_indexes in indexes_by_contract.items():
            ...
            for entity_id_value in manifest_sequence(
                contract.content.get("entity_ids"),
                f"API Contract {contract.content.get('id')}.entity_ids",
            ):
                entity_id = exact_manifest_id(entity_id_value, "API Contract.entity_ids")
                entity = entities.get(entity_id)
                if entity is None:
                    raise ContractCatalogBindingError(
                        f"API Contract {contract.content.get('id')} 缺少实体 {entity_id} 的 binding。"
                    )
                if requirement_kind != "backend.bootstrap" or entity.content.get(
                    "data_source_type"
                ) == source_type:
                    grant(requirement_id, entity, {"/"})
