...
def _page_claims(
    ...
    page = next(
        (
            item
            for item in _dict_items(project_plan.get("frontend_pages"))
            if str(item.get("pageId") or item.get("id") or "") == page_id
        ),
        None,
    )
    if page is None:
    ...
    for related_page in _dict_items(project_plan.get("frontend_pages")):
        related_page_id = str(
