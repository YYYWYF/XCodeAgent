...
def ensure_frontend_menu_entries(frontend_dir: Path, pages: list[dict[str, Any]]) -> dict[str, Any]:
    """只向 main 模板 BIZ_MENUS 顶层追加缺失页面入口。"""
    ...
    return {"status": "succeeded" if not missing_keys else "failed", "path": menus_path.relative_to(frontend_dir.parent).as_posix(), "expectedKeys": expected_keys, "existingKeys": [key for key in expected_keys if key in existing_keys], "injectedKeys": injected_keys, "missingKeys": missing_keys, "error": None if not missing_keys else "部分 ProductPlan 菜单项注入后仍然缺失。"}
