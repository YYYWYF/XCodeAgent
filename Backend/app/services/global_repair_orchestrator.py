"""T6.3 单写者 Global repair 循环；检查与整轮 Unit 执行由受信适配器注入。"""
...
GenerationRoundRunner = Callable[[tuple[UnitRunState, ...]], Awaitable[None]]
...
async def run_global_repair_loop(
    ...
    *,
    ...
    regenerate_round: GenerationRoundRunner,
    now: Callable[[], str] = _utc_timestamp,
) -> GlobalRepairDecision:
    """从已结束的生成轮开始，按完整目标批次修复到 Global 无 Issue 或 Run 失败。

    每个活动 Run 只允许一个调用方驱动此循环。validate_global 只在 Candidate 齐全时
    接收只读 Run，并须返回本次完整检查经 T4.1 归因的决策；本任务可注入 mock。
    regenerate_round 一次接收完整去重目标，须经同一 Controller 跑到 ready/exhausted；
    并发模型策略由 Unit Scheduler 消费 Unit.current_issues，deterministic 策略仍不能进入
    模型 Local retry。
    ...
    while True:
        ...
        targets = tuple(current.unit_states[unit_id] for unit_id in decision.retry_unit_ids)
        await regenerate_round(targets)
        if controller.snapshot.status != "active":
            return decision
        if any(
            controller.snapshot.unit_states[unit.unit_id].generation_status
            not in {"candidate_ready", "round_exhausted"}
            for unit in targets
        ):
            raise ValueError("Generation round runner 必须完成全部目标 Unit 的当前轮次。")
