...
def _context_contract_issues(context: UnitGenerationContext) -> list[ValidationIssue]:
    ...
    for requirement in context.generation_requirements:
        ...
        kind = refs.get("kind")
        if refs.get("capability_id") != requirement.requirement_id or kind not in DELIVERABLE_KINDS:
            issues.append(_fatal_issue(
            ...
            continue
        target_field = DELIVERABLE_TARGET_IDENTITY_FIELD_BY_KIND[kind]
        if _identity(refs.get(target_field)) is None:
            issues.append(_fatal_issue(
                ...
                deliverable_kind=kind, target_identity_field=target_field,
            ))
