...
def resolve_agent_files_root() -> Path:
    """Return the current environment's DevAgent Studio data directory."""
...
def _ensure_environment_root(root: Path | None) -> Path:
    ...
    if environment_root.is_symlink():
        raise AgentFilePathError("DevAgent Studio 环境目录不允许使用符号链接。")
    try:
        environment_root.mkdir(mode=0o700, parents=True, exist_ok=True)
    except OSError as exc:
        raise AgentFilePathError("无法创建 DevAgent Studio 环境目录。") from exc
