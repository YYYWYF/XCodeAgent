...
def mainline_formal_contract_inputs(
    ...
    """把已过门禁的正式产物转换为 Frozen Store 的显式来源输入。"""
    ...
    entity_bindings = _mapping_items(project_plan.get("entity_detail_plans"))
        ...
        entity_bindings=[
            {
                "content": binding,
                "source": {
                    "artifact": _entity_binding_artifact(binding),
...
def _entity_binding_artifact(binding: Mapping[str, Any]) -> str:
    """优先保留 TechnicalPlan 的正式 EntitySourceBinding 引用，否则使用固定路径。"""

    source_binding = binding.get("source_binding")
    if isinstance(source_binding, Mapping):
        path = str(
            source_binding.get("json_path") or source_binding.get("path") or ""
        ).strip()
        if path:
            return path
    return f".xcodeagent/plans/entities/{binding.get('entity_id')}.json"
