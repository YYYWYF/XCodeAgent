...
def _fixture_endpoint_designs(plan: dict) -> list[dict]:
    """为不带工作区路径的测试 Scope 构造当前版 Endpoint Design 快照。"""
    ...
    for contract in plan.get("api_contracts", []):
        ...
        for endpoint in contract.get("endpoints", []):
            ...
            result.append({
                ...
                "implementationDescription": "测试 Endpoint 实现",
                "basedOn": [{
...
def _formal_contract_inputs(
    plan: dict,
    endpoint_designs: list[dict] | tuple[dict, ...] = (),
) -> PlanningFormalInputs:
    """把测试 TechnicalPlan 与当前 Endpoint Design 拆成 Store 正式输入。"""
    ...
    current_endpoint_designs = tuple(endpoint_designs) or tuple(
        _fixture_endpoint_designs(plan)
    )
    inputs = PlanningFormalInputs(
        ...
        endpoint_api_designs=[
            ...
            for design in current_endpoint_designs
        ],
...
def planning_inputs(*, plan=None, baseline=None, required=None, scope=None, context=None):
    ...
    try:
        requirements = resolve_generation_requirements(
            ...
            reuse_facts=facts,
            formal_target=plan,
            endpoint_designs=context.get("endpoint_designs", ()),
        )
    except GenerationRequirementsError:
        # 负向夹具仍应在正式 orchestrator 前置门禁失败，而不是被测试辅助构造提前截断。
        requirements = None
    formal_contract_inputs = _formal_contract_inputs(
        plan,
        context.get("endpoint_designs", ()),
    )
...
def model_tasks(job):
    ...
    backend_stages = {
        backend_stage_by_kind.get(requirement.source_refs.get("kind"))
        for requirement in job.context.generation_requirements
    }
    retained_task_ids = {
        summary.get("id")
        for summary in job.context.dependency_context.get("retained_task_summaries", ())
        if isinstance(summary, dict) and isinstance(summary.get("id"), str)
    }
    available_backend_task_ids = retained_task_ids | {
        f"{job.identity.unit_id}::{stage}"
    ...
    for index, requirement in enumerate(job.context.generation_requirements):
        ...
        target = refs.get("page_id") if kind == "frontend.page" else (
            ...
            else refs.get("data_source_type") if kind == "backend.bootstrap" else refs.get("target_id")
        )
        ...
        stage = backend_stage_by_kind.get(kind)
        task_id = (
            f"{job.identity.unit_id}::{stage}"
            if stage
            else "backend:bootstrap::bootstrap"
            if kind == "backend.bootstrap"
            else f"{job.identity.unit_id}-r{job.identity.generation_round}-a{job.identity.attempt_in_round}-{index}"
        )
        objects_id = f"{job.identity.unit_id}::objects"
        branch_ids = [
            f"{job.identity.unit_id}::{branch}"
            for branch in ("repository", "upstream")
            if f"{job.identity.unit_id}::{branch}" in available_backend_task_ids
        ]
        service_id = f"{job.identity.unit_id}::service"
        # 模型替身必须遵守与生产 Prompt 相同的 Endpoint 分支汇合拓扑。
        if stage in {None, "objects"}:
            dependencies = ()
        elif stage in {"repository", "upstream"}:
            dependencies = (
                (objects_id,) if objects_id in available_backend_task_ids else ()
            )
        elif stage == "service":
            dependencies = tuple(
                branch_ids
                or ([objects_id] if objects_id in available_backend_task_ids else [])
            )
        else:
            dependencies = (
                (service_id,)
                if service_id in available_backend_task_ids
                else tuple(
                    branch_ids
                    or ([objects_id] if objects_id in available_backend_task_ids else [])
                )
