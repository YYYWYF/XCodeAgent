...
class EndpointDesignStalePromotionTests(unittest.TestCase):
    """隔离验证 Endpoint Design 正式输入变化会封死旧 Pending。"""

    def setUp(self) -> None:
        """构造不依赖失效旧 build_context 夹具的 Endpoint Planning 输入。"""
    ...
    def test_endpoint_design_changes_make_pending_stale(self) -> None:
        """revision、映射、来源和 TechnicalPlan 摘要变化都拒绝 Confirm。"""
        ...
        formal_path = build_task_plan_json_path(self.state)
        ...
        for field, mutate in mutations.items():
            ...
            current_inputs = SequentialPlanningInputs.model_validate(payload)

            with self.subTest(field=field):
                self.assertNotEqual(
                    current_inputs.input_fingerprint(),
                    self.inputs.input_fingerprint(),
                )
                ...
                self.assertEqual(result.status, "stale_inputs")
                self.assertEqual(pending_path.read_bytes(), original_pending)
