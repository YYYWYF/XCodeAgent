...
class UnitGenerationRequirementsTests(unittest.TestCase):
    ...
    def test_missing_endpoint_design_and_inconsistent_unit_are_explicit_failures(self) -> None:
        """Endpoint Design 缺失必须阻断，错配的 Unit 节点也不能参与职责计算。"""

        inputs = _inputs()
        inputs["endpoint_designs"] = []
        with self.assertRaises(GenerationRequirementsError) as error:
            resolve_generation_requirements(**inputs)
