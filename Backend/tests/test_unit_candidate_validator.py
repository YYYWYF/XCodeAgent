...
def _requirement(unit_id: str) -> dict:
    ...
    if unit_id == "backend:endpoint:orders-api:orders.list":
        capability = "backend.endpoint_controller:orders-api:orders.list"
        return {
            "requirement_id": capability,
            "description": "实现订单查询 Controller",
            "source_refs": {
                "artifact": "endpoint-api-design", "kind": "backend.endpoint_controller",
                "capability_id": capability, "api_contract_id": "orders-api",
                "endpoint_id": "orders.list", "target_id": "orders.list",
...
class UnitCandidateValidatorTests(unittest.TestCase):
    ...
    def test_backend_deliverable_target_must_be_endpoint_id(self) -> None:
        """Backend Endpoint Candidate 不能用实体身份替代 responsibility 的 Endpoint target。"""

        context = _context("backend:endpoint:orders-api:orders.list")
        task = _task("backend:endpoint:orders-api:orders.list")
        task["deliverables"][0]["target_id"] = "Order"
    ...
    def test_all_backend_responsibilities_accept_only_endpoint_target(self) -> None:
        """五类 Backend responsibility 均以 endpoint_id 作为 Candidate target。"""
        ...
        for kind, requirement_prefix in kinds.items():
            context_payload = _context(unit_id).model_dump(mode="json")
            requirement_id = f"{requirement_prefix}:orders-api:orders.list"
            context_payload["generation_requirements"] = [{
                "requirement_id": requirement_id,
                "description": "实现当前 Endpoint responsibility",
                "source_refs": {
                    ...
                    "data_source_types": ["database", "external_api"],
                },
            }]
            task = _task(unit_id)
            task["deliverables"][0].update({
                "kind": kind,
                "target_id": "orders.list",
                "provides": [requirement_id],
            })

            with self.subTest(kind=kind):
                self.assertEqual(
                    validate_unit_candidate(context_payload, [task]),
                    [],
                )
                entity_target = deepcopy(task)
                entity_target["deliverables"][0]["target_id"] = "Order"
                self.assertIn(
                    "CANDIDATE_REQUIREMENT_TARGET_MISMATCH",
                    {
                        issue.code
                        for issue in validate_unit_candidate(
                            context_payload,
                            [entity_target],
                        )
                    },
    ...
    def test_missing_backend_endpoint_identity_is_input_fatal(self) -> None:
        """Backend requirement 即使带冗余 target_id，也不能绕过 endpoint_id 合同。"""

        unit_id = "backend:endpoint:orders-api:orders.list"
        context = _context(unit_id).model_dump(mode="json")
        context["generation_requirements"][0]["source_refs"].pop("endpoint_id")

        issues = validate_unit_candidate(context, [_task(unit_id)])

        self.assertEqual(
            [issue.code for issue in issues],
            ["UNIT_VALIDATION_REQUIREMENT_TARGET_INVALID"],
        )
        self.assertFalse(issues[0].retryable)
        self.assertEqual(issues[0].category, "input")
        self.assertEqual(
            issues[0].details["target_identity_field"],
            "endpoint_id",
        )

    def test_unknown_deliverable_kind_is_input_fatal(self) -> None:
        """未知 kind 必须在 Candidate 内容校验前 fail closed，不能获得默认 target 字段。"""

        context = _context().model_dump(mode="json")
        context["generation_requirements"][0]["source_refs"]["kind"] = (
            "backend.future_responsibility"
        )

        issues = validate_unit_candidate(context, [_task()])
