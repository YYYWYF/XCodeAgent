...
class AgentRequirementSpecTests(unittest.TestCase):
    ...
    def test_requirement_prompt_asks_before_adding_a_suitable_business_agent(self) -> None:
        """应用适合智能体但用户未提及时，模型必须先建议并等待用户选择。"""

        prompt = _requirements_prompt(
            "创建项目回检管理应用，帮助员工查看回检状态并理解下一步处理要求。"
        )

        self.assertIn("agent suitability check", prompt)
        self.assertIn("one focused yesno question", prompt)
        self.assertIn("concrete proposed agent role", prompt)
        self.assertIn(
            "Do not add an agent_requirements item until the user accepts",
            prompt,
        )
        self.assertIn("do not ask the suitability question again", prompt)

    def test_requirement_prompt_suggests_agents_from_business_context(self) -> None:
        """模型应根据完整业务语义提出建议，普通流程也适用。"""

        prompt = _requirements_prompt("创建一个业务系统，支持处理业务记录。")

        self.assertIn(
            "Interpret intent from the meaning of the full request and context",
            prompt,
        )
        self.assertIn("A plausible business-agent role", prompt)
        self.assertIn(
            "Do not require material value beyond deterministic pages or fixed workflows", prompt
        )
        self.assertIn(
            "Ordinary business workflows are also eligible for a suggestion", prompt
        )

    def test_requirement_prompt_preserves_explicit_agent_choice(self) -> None:
        """明确要求智能体时直接记录，建议获同意或拒绝后均不得重复询问。"""

        prompt = _requirements_prompt("创建一个业务系统，需要智能体协助处理业务记录。")
        self.assertIn(
            "If the user explicitly requested or accepted an agent, populate agent_requirements "
            "without asking a generic suitability question",
            prompt,
        )

        pending_spec = create_requirement_spec("创建一个业务系统，支持处理业务记录。")
        pending_spec["confirmation_status"] = "pending_user_input"
        pending_spec["clarification_questions"] = [
            {"type": "yesno", "question": "是否需要加入智能体能力以协助现有业务？"}
        ]
        for answer, instruction in (
            ("需要", "treat an affirmative answer as explicit permission"),
            (
                "不需要，只保留普通页面",
                "treat a negative answer as a decision to keep agent_requirements empty",
            ),
        ):
            with self.subTest(answer=answer):
                prompt = _requirements_prompt(
                    answer, existing_spec=pending_spec, clarification_round=1
                )
