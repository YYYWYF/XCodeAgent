...
These instructions apply to the whole DevAgent Studio repository. Every Codex agent should read and follow this file before changing code, docs, scripts, or configuration in this project.
...
- Every workflow node that generates or updates a formal artifact consumed by later nodes must stop for explicit user confirmation before downstream logic runs. RequirementSpec and ProductPlan are the sole compound exception: the product planner may consume the validated RequirementSpec draft inside the same "需求文档" node, but both artifacts must be jointly confirmed before UiDesign or any later node runs. Clarification answers only supply missing information and never count as confirmation. Regenerating or revising an artifact requires confirmation of that new version again; node-internal optimizations must not bypass this gate.
