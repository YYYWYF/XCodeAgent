...
- Frontend/backend agent communication must follow the AG-UI protocol. Backend agent endpoints must emit AG-UI-compatible event streams, and frontend message sending plus event parsing must use `@ag-ui/client` and `@ag-ui/core` rather than hand-rolled request, SSE, or event-shape parsing logic.
