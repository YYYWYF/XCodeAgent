...
- **输出**：`.xcodeagent/ui-design/pages/<PageKey>/index.tsx`、运行态临时内联源码 `ui_designs.pages[].code`、`ui-manifest.v5` 的 `pageId/page_key/preview_path/code_path/code_sha256/bindings/verification/status`、`.xcodeagent/specs/ui-designs.json` 和确认状态；`bindings.actions[].uiEffect` 保存已确认的本地界面效果，`bindings.agent_surfaces[]` 保存 Agent 页面载体及固定模板模块、组件、版本和配置摘要证据，正式 JSON 不保存源码或重复 ProductPlan 文案/路由/角色/状态事实，`preview_path` 不是产品正式路由。
...
flowchart TD
    ...
    B -->|"build completed"| I
    B -->|"Agent UI mock_completed / real integration pending"| E
