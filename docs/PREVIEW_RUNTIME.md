...
从欢迎页进入工作台只读取服务状态，不自动启动前端或后端。用户只能通过服务状态抽屉中的“重启服务”主动启动：该操作先停止生成项目的标准前后端服务，再按后端→前端顺序启动，不修改代码。手动重启允许与当前应用正在执行或等待确认的任务并行，不检查 lifecycle、Workflow、独立产品交互或 workspace lease 占用。任何预览维护身份都不再反向阻止普通应用任务启动或恢复；重复重启及诊断修复等预览操作之间仍保持互斥。页面刷新与服务重启是独立动作。成功后刷新当前页面路径；失败时保留日志。缺少后端工程时后端显示无需启动。进程退出或当前就绪探测失败不能继续显示历史成功。

返回欢迎页时立即释放预览维护并在后台停止当前工作区的前后端服务；退出 Electron 前再次停止本次手动重启登记过的所有工作区，避免残留生成项目进程。
...
`/preview-runtime/run` 使用 `forwardedProps.previewRuntime`，字段包括 `workspace`、`action` 和动作所需的 `attemptId`、`planId`、`feedback`。动作是 `get/watch/restart/stop/diagnose/confirm/revise/cancel/leave`。所有字段在 Pydantic 边界校验；不接受日志文件路径。
...
工作区级维护栅栏只序列化同一应用的预览重启、停止和诊断修复，不再阻止 Workflow、workspace lease 或 lifecycle 应用任务启动和恢复。手动 `restart` 与退出清理使用的 `stop` 都绕过应用任务占用；诊断修复仍可拒绝在应用任务占用期间开始，以避免并发修改业务代码。一旦预览维护已经存在，普通应用任务仍可继续。等待修复计划确认只占用预览操作，自身确认/修订/停止允许继续。不同应用互不阻塞。

当前维护身份保存在 `.devagentstudio/runtime/preview-maintenance.json`，终态清空；它只表达维护占用，不是应用配置或阶段完成来源。标准预览当前尝试记录在 `.devagentstudio/runtime/launch/preview-runtime.json`。生命周期和独立对话的待确认投影共同用于阻塞提示。维护任务登记到现有取消注册表，删除工作区前必须等待同步工作实际退出。

平台后端进程重启后，若持久化的 `restart` 或 `stop` 维护身份在当前进程中没有对应运行任务，首次状态读取或新操作会将其识别为孤儿任务：先清理可能残留的标准预览进程，再把未完成启动收口为不可诊断、可直接重试的失败状态并释放维护占用。工作台存续期间即使抽屉关闭也保持无日志的轻量状态订阅，连接中断后按有界间隔重新读取权威快照；只有打开抽屉才传输日志，使工具栏按钮与抽屉始终同步显示状态。
