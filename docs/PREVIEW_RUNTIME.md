...
“重启服务”先停止生成项目的标准前后端服务，再按后端→前端顺序启动，不修改代码。手动重启允许与当前应用正在执行或等待确认的任务并行，不检查 lifecycle、Workflow、独立产品交互或 workspace lease 占用，短时重启维护身份也不阻止普通任务启动或恢复；重复重启及诊断修复等其他预览维护仍保持互斥。页面刷新与服务重启是独立动作。成功后刷新当前页面路径；失败时保留日志。缺少后端工程时后端显示无需启动。进程退出或当前就绪探测失败不能继续显示历史成功。
...
工作台挂载时前端先通过 `get` 读取当前快照，再进入 `watch` 增量订阅，避免长连接首包延迟让未启动服务显示为不可操作。初始快照尚未返回且没有本地占用时仍可提交重启，最终由服务端原子维护栅栏校验是否允许执行。`watch` 在一次有界订阅内仅在状态变化时发布快照；抽屉关闭时保持无日志的轻量订阅，打开后才附带日志，连接失败按一秒间隔重试并重新执行 `get` 校准。服务端维护任务不随状态订阅结束而取消。日志只读取标准运行目录的白名单文件、有界尾部，显示截断标记，并脱敏连接凭据、授权令牌和私钥。安装、构建、启动及 stdout/stderr 都有明确标识。
...
工作区级维护栅栏与工作流运行登记、workspace lease 和 lifecycle 执行登记共享原子锁。诊断修复、启动和停止等维护仍受执行中、停止中或待确认任务约束，并反向阻止其他任务启动或恢复；手动 `restart` 是唯一例外，它可与应用任务并行且不会反向阻断任务。等待修复计划确认仍占用工作区，自身确认/修订/停止允许继续。不同应用互不阻塞。
...
平台后端进程重启后，若持久化的 `start`、`restart` 或 `stop` 维护身份在当前进程中没有对应运行任务，首次状态读取或新操作会将其识别为孤儿任务：先清理可能残留的标准预览进程，再把未完成启动收口为不可诊断、可直接重试的失败状态并释放维护占用。工作台存续期间即使抽屉关闭也保持无日志的轻量状态订阅，连接中断后按有界间隔重新读取权威快照；只有打开抽屉才传输日志，使工具栏按钮与抽屉始终同步显示状态。
