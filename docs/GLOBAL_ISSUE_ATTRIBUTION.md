...
当前生产 DAG 通过 `graph/nodes/task_planning_adapter.py` 进入 Unit Planner、Assembly
和 Global 校验；旧 Scope TaskPreparer 已删除。本任务仍不执行 Global retry、Assembly、
Candidate 状态变更、Round/Attempt 分配、模型裁判或任何文件持久化，也不新增产品 HTTP 入口。
