...
- 当前生产 `Backend/` 已在 RequirementSpec 中识别、归一化、校验和持久化业务智能体需求；新建应用未明确提及智能体但存在具体适配价值时，需求问答会先建议一个用户侧智能体角色并等待用户选择，不会直接创建智能体。ProductPlan v6 继续生成产品级智能体能力、入口页面/操作、交互状态、边界和验收契约；两者复用现有 Markdown 编辑与联合确认链路。
...
- 需求模型识别用户明确提出或在适配建议后明确接受的业务智能体；仅当存在超出确定性页面和固定流程的具体用户价值时，才通过现有 `ask_user` 提出一次角色明确的是/否建议。普通自动化本身不构成智能体需求，用户拒绝后不得重复询问。
