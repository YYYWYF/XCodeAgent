...
flowchart TD
    ...
    L3 --> L4
    L2 -->|"local_fix"| Q5
    L2 -->|"endpoint/data-source mapping change"| D1
    L2 -->|"page/project plan change"| T4
```
...
- **当前提示词**：System Prompt 位于 `agents/data_source/agent.py`；执行 Prompt 位于 `agents/data_source/generator.py::_data_source_generation_prompt`，任务级 Skill 路由和上下文裁剪位于 `agents/data_source/prompt_context.py`。要求逐任务按 Endpoint API 设计中的数据库或 External API 来源读取对应内置技能，只执行批准任务和 allowed paths，严格服从目标 API Contract、字段映射、来源快照与业务规则，代码兼容 Java 8，不在本阶段运行项目级构建/测试。
- **输入**：backend owner 最小任务包、当前目标的 API/Endpoint Design 上下文、工作区和选定技能；不再注入完整 BuildTaskPlan 摘要或 ProjectPlan。
...
- **依赖文件**：`agents/data_source/agent.py`、`agents/data_source/generator.py`、`agents/data_source/prompt_context.py`、`middleware/direct_modification.py`、`agents/workspace_scope.py`、内置 Spring Boot/MyBatis/External API skill。
...
flowchart LR
    ...
    PP[("product-plan.md + json / 产品计划")]
    TP[("technical-plan.md + json / 技术计划")]
    DT[("Endpoint API Design md/json / 动态映射与业务规则")]
...
- DAG Planning ownership 必须区分 lock source 与 session identity evidence：只有 `prepare_build_tasks` 且状态为 `running/stopping` 的 `WorkbenchExecution` 形成 application-level DAG lock；`awaiting_user` 由 PendingPlan 的 `ownerSessionId`/DraftIdentity 处理，`authorization_bootstrap`、普通 `build`、Unit Test、Review、Acceptance 等阶段不形成该锁。`WorkbenchExecution.ownerSessionId` 是稳定的 Chat Session 归属，`threadId` 是 Workflow/Graph execution thread，不能直接当作可见 session identity。active local `SessionExecutionEntry.executionThreadId` 可以跨 phase 关联 lifecycle execution 的 sessionId，但只作为 runtime evidence；Confirm 后进入 authorization/bootstrap 或 Build、页面刷新丢失 local entry、以及 phase lag 都必须保留 owner session 的控制权。当前可见 session 有 active local execution 时，即使 ownership 暂时 conflicted，也不得用 `SessionExecutionLockDock` 替换其 Workflow Debug/End controls；其他 session 的 readonly guard 保持不变。
...
### P0：Endpoint 来源快照与运行凭据必须持续分离

Endpoint API 设计保存确认时的无敏感来源快照，数据源目录后续变化不会主动使设计失效；实际 Build 仍需按 `sourceId` 获取当前凭据。数据源删除或凭据失效会在 Build 时失败，而不会把密钥复制进设计产物。

**建议**：继续保持“稳定设计快照 + 运行时安全解析”的边界，并把失效的 `sourceId`、缺失凭据和连接失败作为可定位的 Build 前置错误展示。
...
| `prepare_build_tasks`                       | 无独立 SystemMessage                                                                            | `agents/main/unit_task_prompt.py::build_unit_generation_prompt`，按 Unit Kind 叠加 `unit_task_rules.py` / `backend_unit_task_rules.py` | Frozen UnitGenerationContext、GenerationRequirements、metadata-only contract catalog、bounded WorkspaceSnapshot、retained owner constraints、Endpoint API Design refs、Local/Global structured feedback；模型只返回单 Unit `tasks` |
