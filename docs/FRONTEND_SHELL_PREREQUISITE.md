...
- 生产：`services/build_unit_skeleton.py`、`services/build_task_reuse.py`、
  ...
  `graph/nodes/tasks.py`、`graph/nodes/task_planning_adapter.py`、
  `agents/main/unit_task_prompt.py`（均位于 `Backend/app`）。
- 新增测试：`Backend/tests/test_frontend_shell_prerequisite.py`。
- 更新测试夹具/断言：`test_confirmed_build_task_plan_loader.py`、
  `test_dag_planning_baseline_gates.py`。
