...
UI 设计是可选节点：选择 UI 设计时，先完成页面视觉稿确认；跳过时，产品规划确认后直接进入技术规划。
...
四个阶段分别确认，但不重复确认同一类事实。以下 JSON 是职责收敛后的简化示意；正式用户文档以 Markdown 为主，JSON 用于内部结构化状态、版本引用和校验。生成候选时使用 `pending_user_confirmation`，用户确认后改为 `confirmed`。
