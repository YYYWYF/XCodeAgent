...
The overlay remains while the selected-page generation or review request runs. The page picker uses `pageId` as the page identity, marks pages with an existing `plans/pages/page--<pageId>.json` detail plan as ready for review, and labels the action as viewing/confirming instead of generating. After that request returns, the workbench switches to the normal chat view so generated artifacts and the detail-review card are displayed through the existing Workflow message UI. The main Workflow hydrates already externalized detail JSON files under `plans/pages/` and `plans/data-source/` when loading ProjectPlan for execution, then the `detail_confirmation` review payload must only include the currently selected page and its direct data-source details; if a selected page has no generated detail plan yet, the payload returns an empty review with `summary.missingSelectedPagePlan=true` so the UI can ask the user to generate that page first.
