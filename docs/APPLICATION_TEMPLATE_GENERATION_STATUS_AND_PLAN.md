...
4. 模板生成 manifest；
5. 模板生成完成门禁；
6. TechnicalPlan 确认后的模板生成 attempt，以及失败后的可重试状态记录。
...
- 仅在用户确认 TechnicalPlan 后执行模板初始化；失败后用户可显式重试同一模板阶段，重启、再次打开和进入工作台都不自动触发。
...
### 5.2 TechnicalPlan 确认后的模板初始化 attempt

只有用户确认 TechnicalPlan 后，前端才启动这一套流程。模板初始化失败后，生命周期保持可重试失败态；用户可显式发起下一次模板 attempt，应用重启、再次打开、进入工作台或查看失败详情都不会自动启动。
