...
当前一等 Deep Agent 是 Frontend Generation、Data Source Generation、Agent Runtime Generation、Database Change、Test、RepairPlanner、SmallTask。requirements、project_planning 和任务规划等 direct ChatModel 节点不加载 Skill；`api_design_readiness_gate` 是纯确定性节点，独立配置流的字段候选加载、保存校验和门禁 revision 复核同样由确定性服务完成。
