...
该节点调用受控 `get_mysql_table_info` 形成 `database-context.v1` 并写入 Graph State。工具先连接 MySQL Server，再检查 `MYSQL_DATABASE` 是否存在；目标库不存在、目标表不存在、字段或约束不足都不是节点错误，而是写入 `actual_schema`、`required_schema`、`resolution_items`、`gaps` 和 `task_intents`。数据库上下文会根据 EndpointDetail 的 field_mappings、differences、processing_logic 和真实库表结构给每个差异标注 `resolution_kind`：`database_change` 交给任务规划生成带 `database_scope` 的数据库任务，`backend_adaptation` 交给后端任务处理转换/null/默认值，`needs_confirmation` 不应进入 build task。只有 `.env` 中 `MYSQL_HOST`、`MYSQL_PORT`、`MYSQL_USER`、`MYSQL_PWD`、`MYSQL_DATABASE` 缺失/非法，或网络、认证、information_schema 访问失败时，节点才返回 `requires_user_input`。`prepare_build_tasks` 只消费该前置上下文，不在任务规划内部再次查库；若当前 scope 来自数据库但缺少 `schema_version=database-context.v1` 且 `status=completed` 的上下文，必须停止并要求重新检查数据库上下文。
