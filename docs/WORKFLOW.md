...
Formal Revision 保留 ChangeImpactAnalyzer 与 Revision Coordinator 对 target、thread policy 和新 Revision Context 的所有权。Coordinator 已决定的 target/context 被转换为 `WorkflowReentryPlan(reason=revision, contextAuthority=REVISION_CONTEXT)`，随后由同一 `WorkflowReentryExecutor` 校验语义摘要并只覆盖新 execution identity；Executor 不重新分析用户意图或变更影响。`CONTINUE_CHECKPOINT` 仅属于 INTERRUPTED/awaiting continuation，operation retry 保留其既有非 exception 业务入口；Stage Restart 不再属于 Generic Recovery，历史 attempt 只允许 fail closed。前端执行当前 Incident 时仍只提交 Backend 签发的 `incidentId/actionId`。
