...
/** 仅将带有 DevAgent Studio 项目标识的安全工作区目录移入系统回收站。 */
async function trashProjectDirectory(workspaceRoot: unknown): Promise<void> {
  ...
  if (protectedRoots.has(pathComparisonKey(projectRoot))) {
    throw new Error('不能删除系统、用户或 DevAgent Studio 数据目录')
  }
  ...
  try {
    ...
    throw new Error('该目录不是由 DevAgent Studio 管理的项目，不能直接删除')
  }
...
/** 创建当前会话记录；前三阶段在锁内分配新 Thread 与全局递增 sequence。 */
...
async function initializePrimaryApplication(): Promise<boolean> {
  ...
  console.log(`DevAgent Studio backend URL: ${backendBaseUrl}`)
...
function handlePrimaryStartupFailure(error: unknown): boolean {
  console.error('Failed to start DevAgent Studio', error)
  app.quit()
  return false
}
...
function handleSecondInstance(): void {
  void focusPrimaryWindowAfterStartup().catch((error) => {
    console.error('Failed to focus the primary DevAgent Studio window', error)
  })
}
