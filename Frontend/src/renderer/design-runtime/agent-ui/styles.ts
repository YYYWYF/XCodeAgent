...
export type AgentUiCssVariables = CSSProperties & {
  '--agent-ui-bg': string
...
/** 把 Ant Design token 投影为固定 Agent UI 的语义 CSS 变量。 */
...
export const AGENT_UI_STYLES = `
.x-agent-ui{box-sizing:border-box;color:var(--agent-ui-text)}
.x-agent-ui *{box-sizing:border-box}
.x-agent-chat{display:grid;grid-template-rows:minmax(0,1fr) auto auto;min-height:0;background:var(--agent-ui-bg)}
...
.x-agent-chat__state{display:flex;min-height:220px;align-items:center;justify-content:center;padding:24px}
.x-agent-chat__state-card{width:min(100%,560px)}
.x-agent-chat__tool,.x-agent-chat__approval{margin-top:12px;border-color:var(--agent-ui-border)}
.x-agent-chat__status{padding:6px 20px;color:var(--agent-ui-text-secondary);background:var(--agent-ui-surface)}
...
.x-agent-conversation__thread--active{padding-inline:10px;border-radius:8px;background:color-mix(in srgb,var(--agent-ui-primary) 10%,transparent)}
.x-agent-conversation__main{min-width:0;display:grid;grid-template-rows:auto minmax(0,1fr)}
.x-agent-conversation__header{display:flex;min-width:0;align-items:center;padding:14px 104px 14px 20px;border-bottom:1px solid var(--agent-ui-border);background:var(--agent-ui-surface)}
...
.x-agent-floating__launcher{position:fixed;z-index:1010;width:56px!important;height:56px!important;border-radius:50%!important;box-shadow:var(--agent-ui-shadow)}
...
.x-agent-floating__panel .ant-card-body{display:grid;grid-template-rows:auto minmax(0,1fr);height:calc(100% - 57px);min-height:0;padding:0;overflow:hidden}
.x-agent-floating__panel .x-agent-chat{height:auto;min-height:0}
.x-agent-floating__simulated{font-size:12px;color:var(--agent-ui-text-secondary)}
@media(max-width:767px){
  .x-agent-chat__messages{padding:16px 12px}.x-agent-message{max-width:94%}.x-agent-composer{padding-inline:12px}.x-agent-chat__approval .ant-space{display:flex;flex-direction:column;align-items:stretch}
  .x-agent-conversation{grid-template-columns:1fr}.x-agent-conversation>.x-agent-conversation__sidebar{display:none}.x-agent-conversation__mobile-menu{display:inline-flex}.x-agent-conversation__header{padding:10px 84px 10px 12px}
  .x-agent-floating__launcher{left:auto!important;right:16px;top:auto!important;bottom:calc(16px + env(safe-area-inset-bottom))}.x-agent-floating__panel{right:16px;bottom:calc(88px + env(safe-area-inset-bottom));width:calc(100vw - 32px);height:min(620px,calc(100vh - 112px))}
}
`
