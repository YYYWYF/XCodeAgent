...
import type { DataSourceFieldType, DataSourceJsonStructure } from '../../typings'
import { FIELD_TYPE_OPTIONS, normalizeJsonFieldTypes } from './jsonFieldTypes'
import { inferJsonStructure, jsonArrayItemPath, jsonPropertyPath, formatJsonSample, parseJsonSampleText, projectStoredJsonStructure, type JsonShape } from './jsonStructure'
...
type FieldTypeProps = {
  structure?: DataSourceJsonStructure | null
...
export function JsonStructureViewer({ value, descriptions = {}, editable = false, onDescriptionChange, fieldTypes = {}, onTypeChange, structure }: { value: unknown; descriptions?: Record<string, string>; editable?: boolean; onDescriptionChange?: (path: string, description: string) => void } & FieldTypeProps): ReactElement {
  const storedResult = useMemo(() => !editable && structure ? projectStoredJsonStructure(structure) : undefined, [editable, structure])
  const result = useMemo(() => storedResult || inferJsonStructure(value), [storedResult, value])
  const displayedDescriptions = storedResult?.descriptions || descriptions
  const effectiveTypes = useMemo(() => normalizeJsonFieldTypes(value, fieldTypes), [value, fieldTypes])
  const [expandedPaths, setExpandedPaths] = useState<Set<string>>(() => new Set(['$']))
  const rows = useMemo(() => renderTreeRows(result.shape, '$', '$', 0, expandedPaths, displayedDescriptions, editable, (path) => setExpandedPaths((current) => { const next = new Set(current); if (next.has(path)) next.delete(path); else next.add(path); return next }), onDescriptionChange, effectiveTypes, onTypeChange), [displayedDescriptions, editable, expandedPaths, onDescriptionChange, result.shape, effectiveTypes, onTypeChange])
  return (
    <div className={cx('data-source-json-structure')}>
      ...
      {result.truncated ? <div className={cx('data-source-json-structure-truncated')}>结构较大，仅展示前 300 个节点或 8 层；完整结构与字段元数据不受显示限制。</div> : null}
    </div>
  )
}
