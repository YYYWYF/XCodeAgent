...
import './ApplicationPlanningRecoveryIncidentCard.less'
...
/** 在聊天历史与输入控制之间渲染唯一的当前 Planning Recovery Incident。 */
export default function ApplicationPlanningRecoveryIncidentCard({
  ...
  const busy = retrying ?? planningTransportBusy(planning)
  ...
  const handleAction = (): void => {
    if (!onAction || busy || !hasAction) return
  ...
  return (
    <section
      ...
      data-testid="application-planning-recovery-incident"
      role="status"
    >
      ...
      {hasAction && onAction ? (
        <Button
          ...
          disabled={busy}
          icon={busy ? <LoadingOutlined spin /> : <RedoOutlined />}
          loading={busy}
...
function FailureSummary({
  ...
  diagnostic?: ApplicationPlanningFailureDiagnostic | null
  message?: string
}): ReactElement {
