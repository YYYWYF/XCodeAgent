...
const outputDirectory = await fs.mkdtemp(
  path.join(os.tmpdir(), 'devagentstudio-commit-before-send-')
)
const outputFile = path.join(outputDirectory, 'commitBeforeSend.test.mjs')
...
try {
  ...
  await build({
    ...
    build: {
      ...
      ssr: path.join(frontendRoot, 'tests/commitBeforeSend.test.ts'),
