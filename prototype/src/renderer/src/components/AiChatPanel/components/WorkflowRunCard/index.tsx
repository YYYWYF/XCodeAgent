...
const ARTIFACT_CONFIRMATION_MAP: Record<string, { docKey: WorkspaceDocKey; title: string; summary: string }> = {
  requirement_spec_confirmation: { docKey: 'requirement-spec', title: '需求文档', summary: '需求文档已生成，请确认内容。' },
  project_plan_confirmation: { docKey: 'project-plan', title: '项目计划', summary: '项目计划已生成，确认后生成构建任务清单。' },
  build_task_plan_confirmation: { docKey: 'build-task-plan', title: '构建任务', summary: '构建任务计划已生成，确认后进入开发阶段。' }
};
...
export default function WorkflowRunCard({
  ...
  // 预设答案（演示一键走流程）：题目带 presetAnswer 时初始化勾选好，用户可直接「下一步」。
  const [answers, setAnswers] = useState<ClarificationAnswers>(() => {
    const initial: ClarificationAnswers = {}
    clarificationQuestions.forEach((question, index) => {
      const preset = (question as WorkflowClarificationQuestion & { presetAnswer?: WorkflowClarificationAnswer }).presetAnswer
  ...
  const safeStep =
    totalQuestions > 0 ? Math.min(clarificationStep, totalQuestions - 1) : 0;
  ...
  // 产物确认（需求/项目计划/构建任务）：CodeX 式授权操作行，非卡片。
  // 消息下方一行：产物已生成状态 + 编辑文档 + 确认保存。
    ...
    return (
      <div className={cx("artifact-auth-bar")}>
        <span className={cx("artifact-auth-status")}>
          ...
          {artifactConfirmation.title}已生成
        </span>
        <span className={cx("artifact-auth-actions")}>
          <Button
            disabled={Boolean(disabled)}
            onClick={() => onDiscard?.(artifactConfirmation.docKey)}
          >
            放弃
          </Button>
          <Button
    ...
    >
      ...
      {(clarificationQuestions.length > 0 || detailReview) && (
        <div className={cx("workflow-clarification")}>
          ...
          {currentQuestion && (
            <>
              <div className={cx("workflow-clarification-stepper")}>
                <Text type="secondary">第 {safeStep + 1} / {totalQuestions} 项</Text>
              </div>
              <div
              ...
              >
                ...
                />
                {currentQuestion.default_assumption && (
                  <Text type="secondary">{currentQuestion.default_assumption}</Text>
                )}
              </div>
