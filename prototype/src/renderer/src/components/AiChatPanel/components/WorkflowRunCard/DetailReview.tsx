...
/** detail_review 确认卡精简为授权行（参考需求分析/项目计划阶段产物授权）：产物在右侧文档区查看/编辑，
 * 对话区只保留「已生成 + 确认保存」。 */
...
export function DetailReviewAuthBar({
  ...
  detailReview: WorkflowDetailReview
  disabled?: boolean
  onConfirm: (submission: WorkflowDetailReviewSubmission) => void
}): JSX.Element {
  const page = detailReview.pages?.[0]
  const agent = detailReview.agents?.[0]
  const summary = detailReview.summary
  const label =
    agent?.name || page?.name || summary?.selectedAgentId || summary?.selectedPageId || '目标'
  ...
  return (
    <div className={cx('artifact-auth-bar')}>
      <span className={cx('artifact-auth-status')}>
        ...
        「{label}」
        {agent ? '智能体设计文档' : hasRelatedEndpoint ? '页面与依赖接口设计' : '页面详细设计'}
        已生成
        {summary
      ...
      <span className={cx('artifact-auth-actions')}>
        <Button
          ...
          onClick={() =>
            onConfirm({ review_status: 'confirmed', target_changes: [], overall_note: '' })
          }
          type="primary"
        >
          确认保存
...
export default function DetailReview({
  ...
  const targets = useMemo(
    () => [...(review.pages || []), ...(review.endpoints || []), ...(review.agents || [])],
    [review]
  )
  const [changes, setChanges] = useState<Record<string, Record<string, unknown>>>({})
  const [overallNote, setOverallNote] = useState('')
  const missingSelectedPagePlan = Boolean(review.summary?.missingSelectedPagePlan)
  const missingSelectedEndpointPlan = Boolean(review.summary?.missingSelectedEndpointPlan)
    ...
    <div className={cx('workflow-detail-review')}>
      <div className={cx('workflow-detail-review-summary')}>
        ...
        <div className={cx('workflow-detail-review-metrics')}>
          ...
          <Tag>
            智能体 <strong>{review.summary?.agent_count || 0}</strong>
          </Tag>
          <Tag>
      ...
      {missingSelectedPagePlan || missingSelectedEndpointPlan || targets.length === 0 ? (
        <Alert
          message={
            message ||
            `目标 ${review.summary?.selectedAgentId || review.summary?.selectedEndpointId || review.summary?.selectedPageId || ''} 还没有生成细节设计，请先生成该目标的 plan。`
          }
          showIcon
          type="warning"
        />
                ...
                <div className={cx('workflow-detail-review-title')}>
                  {target.target_type !== 'page' && (
                    <span className={cx('workflow-detail-review-target-icon')}>
                      {target.target_type === 'agent' ? <RobotOutlined /> : <DatabaseOutlined />}
                    </span>
                  )}
...
function targetKindLabel(targetType: WorkflowDetailReviewTarget['target_type']): string {
  ...
  if (targetType === 'agent') return '智能体'
  return '对象'
}

// 按任务、规则、限制、输入、输出、模型、对话体验、记忆、工具和知识检索审核智能体文档。
function AgentReviewEditor({
  changes,
  disabled,
  onChange,
  target
  ...
  return (
    <div className={cx('workflow-detail-review-fields')}>
      <ReviewTextField
        ...
        value={stringChange(changes.task, target.task)}
      />
      <ReviewListField
        disabled={disabled}
        label="规则"
        onChange={(value) => onChange('rules', value)}
        value={listChange(changes.rules, target.rules)}
      />
      <ReviewListField
        disabled={disabled}
        label="限制"
        onChange={(value) => onChange('limitations', value)}
        value={listChange(changes.limitations, target.limitations)}
      />
      <ReviewListField
        disabled={disabled}
        label="输入"
        onChange={(value) => onChange('inputs', value)}
        value={listChange(changes.inputs, target.inputs)}
      />
      <ReviewListField
        disabled={disabled}
        label="输出"
        onChange={(value) => onChange('outputs', value)}
        value={listChange(changes.outputs, target.outputs)}
      />
      <ReviewTextField
        disabled={disabled}
        label="模型"
        onChange={(value) => onChange('model', value)}
        value={stringChange(changes.model, target.model)}
      />
      <ReviewListField
        disabled={disabled}
        label="对话体验"
        onChange={(value) => onChange('conversation_experience', value)}
        value={listChange(changes.conversation_experience, target.conversation_experience)}
      />
      <ReviewListField
        disabled={disabled}
        label="记忆"
        onChange={(value) => onChange('memory', value)}
        value={listChange(changes.memory, target.memory)}
      />
      <ReviewListField
        disabled={disabled}
        label="工具"
        onChange={(value) => onChange('tools', value)}
        value={listChange(changes.tools, target.tools)}
      />
      <ReviewListField
        disabled={disabled}
        label="知识检索"
        onChange={(value) => onChange('knowledge_retrieval', value)}
        value={listChange(changes.knowledge_retrieval, target.knowledge_retrieval)}
      />
    </div>
  )
}
  ...
  return (
    <div className={cx('workflow-detail-review-fields')}>
      ...
      <ReviewListField
        disabled={disabled}
        label="基本布局"
        onChange={(value) => onChange('basic_layout', { ...layout, structure: value })}
        value={listChange(objectValue(changes.basic_layout).structure, layout.structure)}
      />
      <ReviewSummaryField
        disabled={disabled}
        label="页面布局设计"
        onChange={(value) =>
          onChange('layout_design', parseLayoutDesignSummary(value, target.layout_design))
        }
      ...
      <ReviewSummaryField
        disabled={disabled}
        label="主要操作交互"
        onChange={(value) => onChange('operation_interactions', parseOperationSummary(value))}
        value={operationSummary(
          recordListChange(changes.operation_interactions, target.operation_interactions)
        )}
      />
      <ReviewSummaryField
        disabled={disabled}
        label="状态反馈"
        onChange={(value) => onChange('state_feedback', parseStateFeedbackSummary(value))}
      ...
      <ReviewSummaryField
        disabled={disabled}
        label="API 依赖"
        onChange={(value) => onChange('api_dependencies', parseApiDependencySummary(value))}
      ...
      <ReviewSummaryField
        disabled={disabled}
        label="响应字段绑定"
        onChange={(value) => onChange('response_bindings', parseResponseBindingSummary(value))}
      ...
      <ReviewSummaryField
        disabled={disabled}
        label="页面跳转与依赖"
        onChange={(value) => onChange('page_navigation', parseNavigationSummary(value))}
        value={navigationSummary(recordListChange(changes.page_navigation, target.page_navigation))}
      />
      <ReviewListField
        disabled={disabled}
        label="页面权限"
      ...
      <ReviewSummaryField
        ...
        label="操作可见性"
        onChange={(value) =>
          onChange('operation_visibility', parseOperationVisibilitySummary(value))
        }
        value={operationVisibilitySummary(
          recordListChange(changes.operation_visibility, target.operation_visibility)
        )}
      />
      <ReviewListField
        disabled={disabled}
...
function EndpointReviewEditor({
  ...
  const shouldResolveDataOrigin = isNeedsUserConfirmationDataOrigin(target.data_origin)
  return (
    <div
      className={cx(
        'workflow-detail-review-fields',
        shouldResolveDataOrigin && 'workflow-detail-review-fields-data-origin-pending'
      )}
    >
      <ReviewSummaryField
        disabled={disabled}
      ...
      {shouldResolveDataOrigin ? (
        ...
        <ReviewSummaryField
          ...
          label="二、数据来源"
          onChange={(value) =>
            onChange('data_origin', parseDataOriginSummary(value, target.data_origin))
          }
      ...
      <ReviewSummaryField
        ...
        value={jsonSummary(objectChange(changes.endpoint_decision, target.endpoint_decision))}
      />
      <ReviewSummaryField
        disabled={disabled}
        label="三、接口设计"
        onChange={(value) => onChange('interface_design', parseJsonObject(value))}
        value={jsonSummary(objectChange(changes.interface_design, target.interface_design))}
      />
      <ReviewListField
        disabled
...
function ReviewListField({ disabled, label, onChange, value }: FieldProps<string[]>): ReactElement {
  return (
    <label
      className={cx('workflow-detail-review-field', 'workflow-detail-review-field-structured')}
    >
      <Text type="secondary">{label}</Text>
      <TextArea
        autoSize={{ minRows: 4, maxRows: 4 }}
...
function objectChange(changed: unknown, initial: unknown): Record<string, unknown> {
  return isRecord(changed) ? changed : objectValue(initial)
}

function recordListChange(changed: unknown, initial: unknown): Array<Record<string, unknown>> {
  const changedItems = recordItems(changed)
  return changedItems.length > 0 || Array.isArray(changed) ? changedItems : recordItems(initial)
}
...
function isNeedsUserConfirmationDataOrigin(value: unknown): boolean {
  ...
  const hasPendingDifference =
    ...
    origin.differences.some(
      (item) => isRecord(item) && String(item.resolution_kind || '') === 'needs_user_confirmation'
    )
...
function parseDataOriginSummary(value: string, current: unknown): Record<string, unknown> {
...
function splitPair(value: string, separator: string): [string, string] {
  ...
  return index >= 0
    ? [value.slice(0, index).trim(), value.slice(index + separator.length).trim()]
    : [value.trim(), '']
}
...
function parseLayoutDesignSummary(value: string, current: unknown): Record<string, unknown> {
  ...
  summaryLines(value).forEach((line) => {
    ...
    if (line.startsWith('主要内容呈现：')) {
      next.primary_content_presentation = line.replace('主要内容呈现：', '').trim()
      readingRegions = false
      return
    }
    ...
    if (readingRegions) {
      const [name, responsibility] = splitPair(line.replace(/^-+\s*/, ''), '：')
      regions.push({
        name: name || '页面区域',
...
function parseStateFeedbackSummary(value: string): Array<Record<string, unknown>> {
  return summaryLines(value).map((line) => {
...
function parseApiDependencySummary(value: string): Array<Record<string, unknown>> {
  return summaryLines(value).map((line) => {
...
function parseResponseBindingSummary(value: string): Array<Record<string, unknown>> {
  return summaryLines(value).map((line) => {
    const [endpoint, detail] = splitPair(line, '：')
...
function navigationSummary(value: unknown): string {
  const lines = recordItems(value).map((item) => {
    ...
    const target = String(item.targetPageId || item.target_path || '待补充目标页面')
    const behavior = String(item.behavior || item.description || '待补充行为')
...
function parseOperationVisibilitySummary(value: string): Array<Record<string, unknown>> {
  return summaryLines(value).map((line) => {
    const [action, detail] = splitPair(line, '：')
