...
import { useChatSessions, type RelatedEndpointContext } from './hooks/useChatSessions'
...
import { isPendingWorkspaceCodeChange, resolvePlanningArtifactKey } from './panelHelpers'
import {
  agentArtifactId,
...
type DevelopmentTemplateTarget =
  ...
  | { kind: 'agent'; artifactId: string; agent: DevelopmentPlanningAgent }
  | {
      kind: 'endpoint'
      artifactId: string
      apiContractId: string
...
function developmentWorkflowArtifactId(workflow?: WorkflowRunPayload): string {
  ...
  if (objectId && detailTargetType === 'business-object') {
    return businessObjectArtifactId(objectId)
  }
...
export default function AiChatPanel({
  ...
  /**
   * 产物面板候选：页面、实体、接口与智能体；已完成、进行中或已被后台任务接管的产物不可再次发起，
  ...
  const composerMentionItems: ComposerArtifactTarget[] = [
    ...
    ...developmentPlanningAgents.map((agent) => {
      const artifactId = agentArtifactId(agent.id)
      const occupancy = resolveArtifactOccupancy(artifactId)
      const delivered = developmentArtifactStatusById[artifactId] === 'completed'
      const blocked = occupancy.blocked || delivered
      return {
        artifactId,
        kind: 'agent' as const,
  ...
  const buildTemplateTargetFromMention = (
    ...
    if (mention.kind === 'agent') {
      const agent = developmentPlanningAgents.find((candidate) => candidate.id === mention.agentId)
      return agent ? { kind: 'agent', artifactId: mention.artifactId, agent } : undefined
    }
  ...
  const startArtifactWorkflow = async (target: DevelopmentTemplateTarget): Promise<void> => {
    ...
    if (target.kind === 'agent') {
      ...
      const endpointKey = `${target.apiContractId}:${target.endpointId}`
      setActiveArtifactTab('page-source')
      setInteractingDetailTargetKey(endpointKey)
      setActiveDetailTarget({
        type: 'endpoint',
        apiContractId: target.apiContractId,
        endpointId: target.endpointId,
