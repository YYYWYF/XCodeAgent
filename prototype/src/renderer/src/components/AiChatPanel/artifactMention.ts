 ...
 * 开发阶段通过「产物」按钮直接选择页面 / 接口 / 实体 / 智能体发起实施（不再经输入框 @ 键入）；
 * 这里集中维护候选目标的状态描述与历史消息中 @ 提及的轻格式渲染，便于独立复用与测试。
 */

/** 产物候选与直接发起共用的目标描述。 */
export type ComposerArtifactTarget = {
  /** 领域产物 ID（page:xxx / endpoint:contract:endpoint / business-object:xxx / agent:xxx）。 */
  artifactId: string
  kind: 'business-object' | 'page' | 'endpoint' | 'agent'
