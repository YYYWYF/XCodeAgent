...
// 从问题的 default_answer 构建表单初始答案（演示案例预填；无 default_answer 保持空）。
function defaultAnswers(questions: WorkflowClarificationQuestion[]): WorkflowClarificationAnswers {
  const answers: WorkflowClarificationAnswers = {}
  questions.forEach((question, index) => {
    const defaultAnswer = (question as WorkflowClarificationQuestion & {
      default_answer?: WorkflowClarificationAnswer
    }).default_answer
    if (defaultAnswer !== undefined) {
      answers[questionKey(question, index)] = defaultAnswer
    }
  })
  return answers
}
    ...
    >
      ...
      {!editingRequirement ? (
        <Form
          ...
          form={form}
          initialValues={{ answers: defaultAnswers(questions) }}
          layout="vertical"
