// Mock preload：在 main.tsx 之前注入，构造 window.devAgentStudio（替代 Electron IPC）
  ...
  const suppressed = (...args: unknown[]): boolean =>
    args.some((a) => {
      ...
      return (
        ...
        text.includes('[devagentstudio-mock]')
      )
    })
...
const USER_APPLICATIONS_KEY = 'devagentstudio:prototype:user-applications'
...
// window.devAgentStudio 的 mock 实现（覆盖 preload/index.ts 暴露的全部命名空间）。
const devAgentStudio = {
  isElectron: false,
  ...
  workspace: {
    ...
    // 拒绝复用已有应用的工作区目录，与桌面端“已有 DevAgent Studio 应用目录不能复用”的规则一致；
    // 手动输入预置演示应用路径同样会被拦截。
    createProjectDirectory: (payload: { workspacePath?: string }) => {
      ...
      if (occupied) {
        return Promise.reject(
          new Error(`该目录已被应用「${occupied.name}」使用，已有 DevAgent Studio 应用目录不能复用。`)
        )
      }
...
;(window as unknown as { devAgentStudio: unknown }).devAgentStudio = new Proxy(devAgentStudio, {
...
// 防御：真实代码里仍有少量直接访问 window.devAgentStudio 的子属性（可选链已容错）。
// 这里显式声明已就绪。
